...
export async function processVault(options: BatchProcessOptions): Promise<void> {
    ...
    for (const filePath of vaultFiles.markdown) {
        ...
        if (cache[relativePath] && cache[relativePath].mtimeMs === mtimeMs) {
            try {
                ...
                const cachedNote = JSON.parse(cachedContent);
                if (!cachedNote.frontmatter) {
                    cachedNote.frontmatter = {};
                }
                if (!cachedNote.stats) {
                    cachedNote.stats = { words: 0, characters: 0, codeBlocks: 0, headings: 0, images: 0, links: 0, readingTime: 0 };
                }
    ...
    for (const note of notes) {
        ...
        const safeLinks = note.links || [];
        const safeBacklinks = note.backlinks || [];
        const neighborSlugs = new Set([...safeLinks, ...safeBacklinks]);
        ...
        // Get Tags first for the local graph
        let tags: string[] = [];
        ...
        note.localGraph = {
            ...
            edges: [
                ...(note.links || []).map((targetSlug: string) => ({
                    source: note.slug,
                    target: targetSlug,
                    type: 'wikilink' as any,
                })),
                ...(note.backlinks || []).map((sourceSlug: string) => ({
                    source: sourceSlug,
                    target: note.slug,
                    type: 'wikilink' as any,
                })),
    ...
    await writeFile(
        path.join(outputPath, '_index.json'),
        JSON.stringify(globalIndex, null, 2)
    );

    // Generate Search Index
    const searchIndex = notes.map((note, index) => {
        let tags: string[] = [];
        if (Array.isArray(note.frontmatter.tags)) {
            tags = note.frontmatter.tags.map(String);
        } else if (typeof note.frontmatter.tags === 'string') {
        ...
        const plainText = extractText(note.contentAst).replace(/\s+/g, ' ').trim();
    ...
    for (const note of notes) {
        ...
        const safeLinks = note.links || [];
        for (const targetSlug of safeLinks) {
