...
export async function parseMarkdown(
  ...
  try {
    ...
    const rawLinks = extractLinkSlugs(linkDetails);

    // Resolve short slugs to full slugs using availableSlugs
    const links = rawLinks.map((rawSlug: string) => {
      let checkSlug = rawSlug;
      if (checkSlug.endsWith('.md')) checkSlug = checkSlug.slice(0, -3);

      if (checkSlug.startsWith('.')) {
        checkSlug = resolveRelativePath(checkSlug, slug);
      }

      if (opts.availableSlugs.has(checkSlug)) return checkSlug;
      const match = Array.from<string>(opts.availableSlugs).find((s: string) => s.endsWith(`/${checkSlug}`));
      return match || checkSlug;
    });

    note.linkDetails = linkDetails;
    note.links = Array.from(new Set(links)); // Deduplicate any resolved overlaps
...
export function buildBacklinks(notes: NoteContent[]): void {
  ...
  // Initialize backlinks and ensure links arrays exist
  for (const note of notes) {
    note.backlinks = [];
    if (!note.links) {
      note.links = [];
