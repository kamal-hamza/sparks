...
describe('calloutPlugin', () => {
  test('converts info callout to sparks-callout', async () => {
    const content = '> [!info]\n> This is important.';
    const note = await parseMarkdown(content, 'test.md', {
    ...
    const callout = findElement(note.contentAst, el =>
      el.tagName === 'sparks-callout'
    );
    expect(callout).toBeDefined();
    expect(callout?.properties?.type).toBe('info');
  });
  ...
  test('converts warning callout', async () => {
    ...
    const callout = findElement(note.contentAst, el =>
      el.tagName === 'sparks-callout' && el.properties?.type === 'warning'
    );
    expect(callout).toBeDefined();
  });
  ...
  test('converts note callout', async () => {
    ...
    const callout = findElement(note.contentAst, el =>
      el.tagName === 'sparks-callout' && el.properties?.type === 'note'
    );
    expect(callout).toBeDefined();
  });
  ...
  test('converts tip callout', async () => {
    ...
    const callout = findElement(note.contentAst, el =>
      el.tagName === 'sparks-callout' && el.properties?.type === 'tip'
    );
    expect(callout).toBeDefined();
  });
  ...
  test('handles callout with title', async () => {
    ...
    expect(callout).toBeDefined();
    expect(callout?.properties?.title).toBe('Custom Title');
  });
