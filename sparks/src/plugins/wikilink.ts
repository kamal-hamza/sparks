                    ...
                    visit(tree, 'text', (node: any, index: number | undefined, parent: any) => {
                        ...
                        text.replace(wikilinkRegex, (match: string, content: string, offset: number) => {
                            ...
                            if (isAsset) {
                                ...
                                // Resolve short slug if possible
                                const availableSlugs: Set<string> = file?.data?.parserOpts?.availableSlugs;
                                if (availableSlugs && !availableSlugs.has(slug)) {
                                    const match = Array.from<string>(availableSlugs).find((s: string) => s.endsWith(`/${slug}`));
                                    if (match) slug = match;
                                }
